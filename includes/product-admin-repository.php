...
function productAdminList(mysqli $conn, int $limit = 200): array
{
    $result = $conn->query("SELECT p.id, p.name, p.slug, p.sku, p.base_price, p.stock_status, p.is_active, p.is_featured,
...
function productAdminSave(mysqli $conn, array $data): array
{
    ...
    try {
        ...
        if (isset($data['stock_quantity']) && $data['stock_quantity'] !== '') {
            require_once __DIR__ . '/inventory-repository.php';
            inventorySetProductQty($conn, $productId, (int) $data['stock_quantity']);
            if ((int) $data['stock_quantity'] > 0 && $stockStatus === 'preorder') {
...
/**
 * Lấy ID sản phẩm bán chạy nhất.
 *
 * @return int[]
 */
function productAdminGetTopSellerIds(mysqli $conn, int $limit = 6): array
{
    $items = productGetBestSellers($conn, $limit);
    return array_map(static fn (array $row) => (int) $row['id'], $items);
}

/**
 * Đánh dấu sản phẩm chủ lực (is_featured) theo số lượng đã bán.
 */
function productAdminApplyFeaturedFromSales(mysqli $conn, int $limit = 6): array
{
    $limit = max(1, min(12, $limit));
    $ids = productAdminGetTopSellerIds($conn, $limit);
    if ($ids === []) {
        return ['ok' => false, 'message' => 'Chưa có đơn hàng — không thể đề xuất theo lượt mua.'];
    }

    $conn->begin_transaction();
    try {
        if (!$conn->query('UPDATE products SET is_featured = 0')) {
            throw new RuntimeException('reset featured');
        }
        $stmt = $conn->prepare('UPDATE products SET is_featured = 1 WHERE id = ?');
        if (!$stmt) {
            throw new RuntimeException('prepare featured');
        }
        foreach ($ids as $id) {
            $stmt->bind_param('i', $id);
            $stmt->execute();
        ...
        return [
            'ok' => true,
            'message' => 'Đã gán ' . count($ids) . ' sản phẩm chủ lực theo lượt mua.',
            'product_ids' => $ids,
        ];
    } catch (Throwable $e) {
        $conn->rollback();
        return ['ok' => false, 'message' => 'Không thể cập nhật sản phẩm chủ lực.'];
    }
}
